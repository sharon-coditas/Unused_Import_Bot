 ...
 */
public class UnusedImportScanners {
    ...
    public static String listUnusedImports(final String contents) throws FormatterException {
        Context context = new Context();
        JCCompilationUnit unit = parse(context, contents);
        List<String> unusedImportsList = new ArrayList<>();
        if (unit == null) {
        ...
        for (JCImport jcImport : unit.getImports()) {
            String sname = getSimpleName(jcImport);
            if (!isUnused(unit, scanner.usedNames, scanner.usedInJavadoc, jcImport, sname)) {
                continue;
            }
            unusedImportsList.add(jcImport.toString());
        }
       return (unusedImportsList.toString().replace("[","").replace(", ","").replace("]",""));
    }
